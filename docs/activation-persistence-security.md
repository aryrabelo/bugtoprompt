# snap-prompt — Activation, Persistence & Key Safety
...
`import { SnapPrompt }` (React) or paste a `<script>` / console snippet, and the
...
| Path | How | When |
|---|---|---|
| **React import** | `import { SnapPrompt } from "snap-prompt"` → `<SnapPrompt />` (no required props) | SPA hosts you control (e.g. gerarposts) |
| **Script tag** | `<script src="…/snap-prompt.global.js" defer></script>` (self-mounts) | Any site, incl. server-rendered / "old" multi-page apps |
...
2. **Global** — `window.__SNAP_PROMPT__ = { baseUrl?, modes?, projectId?, assemblyAiKey?, streamingToken? }` (this is what a console snippet sets — §3).
3. **Meta** — `<meta name="snap-prompt-base" content="/api">`.
4. **Server config** — `GET {base}/snap-prompt/config` → `{ modes, defaultMode, projectId }`. The server decides what's enabled.
5. **Safe fallback** — if no backend answers: `modes = ["clipboard","download"]`, a local no-backend client (no secrets, always works).

Result: `<SnapPrompt />` with nothing set still works (capture → copy/download the
...
- **Dev Mode** (`{base}/snap-prompt/config` advertises `env:"dev"`, server holds
...
- **Production** (no key endpoint exposed — by design, to avoid leakage): the
  ...
  - **Unlock on demand, via the console:** a developer pastes a snippet that
    ...
    window.__SNAP_PROMPT__ = { assemblyAiKey: prompt("AssemblyAI key?") };
    import("https://…/snap-prompt.global.js");
    ```
...
- A **`SessionStore`** persists the in-progress capture: `sessionId`, `startedAt`,
  ...
  **transcript** → `localStorage` (key `snap-prompt:session`). Screenshot **blobs**
...
- `dist/snap-prompt.global.js` — IIFE, bundles React+ReactDOM, self-mounts from
  `data-*` / `window.__SNAP_PROMPT__` / server config.
- `dist/snap-prompt.css` — compiled, self-contained overlay styles (own tokens),
...
- **Config home:** extension storage (`chrome.storage.local`/`sync`) holds the
  resolved config; it is injected as `window.__SNAP_PROMPT__` before boot, so the
...
- **Reuse:** the extension wraps the **same** `snap-prompt.global.js` + CSS — it
...
activation path — props, `data-*`, `window.__SNAP_PROMPT__`, server config, or
